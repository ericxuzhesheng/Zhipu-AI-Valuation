...
\begin{frame}{两件事勾稽：争夺同一个开发者工作流}
  \begin{itemize}
    \item ZCode 回应争议、承诺开源：修复数据控制信任，守住现有客户。
    \item MiniMax 发布可审查的 CLI：以工具透明度降低替代方案的评估门槛。
    \item 共同传导：代码控制信心 $\rightarrow$ 采购与试用 $\rightarrow$ 迁移、留存及付费使用。
    \item CLI 支持其他模型，工具获客未必转化为 MiniMax API 收入。
  \end{itemize}
  \begin{block}{商业联系是本文的竞争解释}
    信任争议让透明度更有竞争价值，开源则提供可供评估的替代入口。尚无证据证明针对性发布、实际客户迁移或收入转移；股价因果须另行检验。
  \end{block}
\end{frame}

\begin{frame}{最后的结论}
  \begin{block}{一句话}
    能力超预期推动重估，随后预期溢价部分回归；本周反弹与分化显示调整并非单向。ZCode 信任修复与 MiniMax 开源共同指向开发者入口竞争，最终要看留存和付费兑现。
  \end{block}
  \vspace{0.4em}
  \textbf{论文的三个贡献}
  \begin{itemize}
