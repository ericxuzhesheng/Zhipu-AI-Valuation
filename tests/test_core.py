...
class EventPanelTests(unittest.TestCase):
    def test_event_catalog_input_drives_panel(self) -> None:
        ...
        self.assertEqual(int(panel["included"].sum()), 10)
    ...
    def test_panel_summary_has_expected_total_row(self) -> None:
        ...
        self.assertEqual(int(total["n_events"]), 10)
        self.assertAlmostEqual(float(total["mean_reaction_car_pct"]), 5.92)
        self.assertAlmostEqual(float(total["mean_drift_car_pct"]), 2.34)
