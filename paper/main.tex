...
By 11~September its shares were 6.8 times the IPO price, implying US\$47.3bn of equity value. Results released
after the 31~August close showed H1 revenue of RMB~953.9m, up 399.7\%; API revenue contributed 86.5\%, while operating
...
and gives a probability-weighted HK\$88, or 11.1\% of the September close. Private transactions imply
HK\$240--457; the closest listed peer implies HK\$257 on matched LTM revenue. The reverse DCF requires about US\$60.5bn of 2035
...
pre-profit issuers. At the 11~September close its shares were 6.8 times the IPO price, giving a post-placement
market capitalisation of US\$47.3bn. Against US\$209.5m of LTM revenue through 2026H1, that is an
equity-value-to-revenue multiple of 226.0$\times$
...
then re-rated to HK\$793 by 11~September~2026 ($\approx +582\%$ versus IPO; realised annualised volatility
near 189\%; Figure~\ref{fig:price}). The company placed 19.78m new shares on 13~July at HK\$1{,}588,
raising HK\$31.375bn net and increasing issued shares from 445.843m to 465.623m
\citep{placement2513}. MiniMax, listing one day later at HK\$165, closed 11~September at HK\$270 after a much
...
\begin{table}[H]
\centering
\caption{Three-scenario DCF (USD unless noted; per-share in HKD). Market row at the 11~September close. Operating assumptions are unchanged.}
\label{tab:dcf}
\begin{tabular}{lcccc}
\toprule
...
\midrule
\textit{Market (2026-09-11)} & --- & --- & \textit{US\$47.3B} & \textit{793} \\
\bottomrule
\end{tabular}
\end{table}

The probability-weighted intrinsic value is HK\$88 per share---roughly \textbf{11.1\% of the market price}.
...
We invert the model to ask what justifies the US\$47.3bn equity value while holding FY2026E revenue fixed.
At WACC~13.5\% and a 40\% terminal margin, the price requires \textbf{about US\$60.5bn of 2035 revenue}:
2027 growth of roughly 130\%, followed by a path that still compounds at 64.1\% from 2026 to 2035.
Figure~\ref{fig:revheat} grids WACC (10--20\%) against terminal margin (20--50\%). Even the most favourable
cell (10\%/50\%) requires about US\$19bn, while 15\%/40\% requires US\$85bn. The reverse DCF is therefore
...
\caption{Reverse-DCF sensitivity: 2035 revenue (US\$bn) required to justify the US\$47.3bn equity value
...
\label{sec:rel}
We divide each listed lab's 11~September equity value by LTM revenue through 2026H1. Zhipu's RMB~1{,}487.3m
(FY2025 less H1~2025 plus H1~2026), or US\$209.5m at RMB7.1/US\$, gives \textbf{226.0$\times$}.
MiniMax's US\$12.09bn equity value and US\$165.2m LTM revenue give \textbf{73.2$\times$}. Private transactions imply
...
giving HK\$240--457 per share. MiniMax's LTM multiple applied to Zhipu's LTM revenue gives HK\$257.
...
On the matched basis, Zhipu's multiple is 3.09 times MiniMax's, so the peer supports far less of the quote
...
intrinsic value; the rest is time value on a fat right tail, amplified by roughly 189\% annualised volatility
...
and MiniMax's 73.2$\times$ LTM multiple gives about HK\$257 when applied to Zhipu's LTM revenue. That anchor is
67.6\% below Zhipu's HK\$793 close. Public markets value both listed labs richly, but Zhipu still carries a
3.09-fold multiple premium to MiniMax, making its price unusually dependent on a durable right-tail option.
...
\caption{Rebased prices (IPO~$=100$, log scale, through 2026-09-11). Orange dashed lines mark capability
...
a HK\$793 September close. MiniMax's public multiple implies only HK\$257 on a matched LTM basis. Listed
scarcity affects both firms, but Zhipu still trades at 3.09 times MiniMax's revenue multiple. The reverse DCF states the wager---about
US\$60.5bn of 2035 revenue.
...
label. The main limitations are the small event sample, incomplete post-results windows, and an uncertain path
from API volume to durable cash flow.

\subsection*{Disclosure update: information through 13 September}
The information cutoff is 13~September~2026, a Sunday; prices remain at the 11~September close.
The original five-event cohort and bootstrap remain fixed at 31~August. Supplementary September windows
are reported separately in \texttt{UPDATE\_2026-09-11.md}.

\textbf{Financing and valuation treatment.} Zhipu's 13~September announcement records agreements signed
on 12~September for up to 21,965,000 new H shares at HK\$714 and RMB20.14bn of US-dollar-settled,
zero-coupon convertible bonds \citep{zhipufinancing2026}. The placement price is 9.96\% below the
11~September close and 19.95\% below the preceding five-day average. Expected placement net proceeds
are HK\$15,664.13m; expected bond net proceeds are US\$3,010.77m. The initial conversion price is
HK\$892.50, 12.55\% above the closing share price. The bonds are issued at 100.5\% of principal and
mature on 16~September~2027, with the ordinary conversion period beginning on 1~July~2027.

\pagebreak
The roughly US\$5bn headline describes financing arranged, not cash established as received by this
cutoff. Placement completion is expected on 16~September subject to conditions including listing approval;
bond issuance is also conditional, and neither transaction depends on the other completing. The announcement
confirms 465,623,090 existing shares. Full placement would increase this to 487,588,090; subsequent full
conversion at the initial price would bring it to 513,953,116. These imply approximately 4.50\% and 9.40\%
reductions in existing holders' percentage ownership, respectively, before any other changes. They are
ownership calculations, not predicted price declines.

Accordingly, the model retains the existing share count and June-balance-plus-July-placement net-cash
bridge; September proceeds are not treated as settled. After closing, a revised bridge would have to record
new cash alongside the debt claim and equity dilution. Adding all convertible proceeds to equity value
without a corresponding liability or conversion treatment would overstate value. Zero coupon eliminates
regular interest payments, not the one-year repayment obligation if the bonds remain outstanding.
The company allocates 60\% of proposed net proceeds to next-generation GLM development, fully self-training
and computing infrastructure. This supports technical investment capacity, but does not establish an
operating return. The HK\$88 weighted DCF remains a reference under the original assumptions, not a
...
\textbf{Why the price fell back.} Between 31~August and 11~September, Zhipu falls from HK\$1,195 to
HK\$793 (33.6\%), while MiniMax falls from HK\$349 to HK\$270 (22.6\%). Common weakness is consistent
with sector repricing, but the roughly 11-percentage-point difference does not identify a company-specific
cause. Zhipu's H1 gross-margin decline to 26.4\%, widening operating and adjusted losses, and still-high
226.0x LTM multiple give concrete reasons why revisions to commercialization expectations could move the
price sharply. This is an interpretation consistent with disclosed fundamentals, not a causal estimate.
The GLM-5.3-Flash window also shows that a positive two-day reaction (+9.1\%) can coexist with a negative
subsequent drift (-25.9\%). The September financing discount and potential dilution reveal the terms of
additional capital, but the formal announcement follows the measured drawdown. It cannot establish why
that earlier decline occurred, and no post-announcement trading day exists within this cutoff.
...
\label{sec:revheat}
Table~\ref{tab:revheat} reports the 2035 revenue (US\$bn) required to justify the US\$47.3bn equity value
across a grid of WACC and terminal-margin assumptions, underlying Figure~\ref{fig:revheat}.

\begin{table}[H]
\centering
\caption{Reverse-DCF sensitivity: required 2035 revenue (US\$bn) to justify US\$47.3bn equity.}
\label{tab:revheat}
