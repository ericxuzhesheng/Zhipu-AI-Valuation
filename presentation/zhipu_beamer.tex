  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        ...
        \item 智谱 LTM 收入 US\$209.5m，对应 340.5x；MiniMax 为 94.6x。
        \item MiniMax 倍数应用于智谱 LTM 收入，对应每股约 HK\$332。
      \end{itemize}
      \begin{block}{估值含义}
        两家上市公司都享有高估值，但智谱的收入倍数仍是 MiniMax 的 3.60 倍。
      \end{block}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{三层可比回答三个不同的问题}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \textbf{读图顺序}
      \begin{itemize}
        \item 上市基础模型同行采用同一 LTM 规则：智谱 340.5x，MiniMax 94.6x。
        \item 港股邻近组帮助判断本地 AI 资产的估值位置，但收入结构不同。
        \item 商业化参照组显示成熟 AI 软件的收入倍数，不参与核心中位数。
      \end{itemize}
      \smallnote{智谱倍数约为 MiniMax 的 3.60 倍。私募公司使用收入 run-rate，保留为单独的交易参照。}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.32\textwidth}
      ...
      \begin{itemize}
        ...
        \item MiniMax 的 94.6x 倍数显示板块有共同溢价，但远低于智谱的 340.5x。
