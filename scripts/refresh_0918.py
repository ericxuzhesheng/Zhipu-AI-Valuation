...
for company, filename in [('MiniMax', 'MiniMax_daily.csv'), ('Wenge AI', 'WengeAI_daily.csv')]:
    ...
    ratio = float(prices.loc[20260918, 'close']) / float(prices.loc[old_date, 'close'])
    comps.loc[mask, 'equity_value_bn'] = float(comps.loc[mask, 'equity_value_bn'].iloc[0]) * ratio
    comps.loc[mask, 'valuation_date'] = '2026-09-18'
    comps.loc[mask, 'valuation_source_url'] = 'https://web.ifzq.gtimg.cn/appstock/app/hkfqkline/get'
    note = ' Price refreshed to 2026-09-18; share-count and FX basis carried forward from the 2026-08-31 snapshot.'
...
r.write_sensitivity_chart()
r.write_base_projection_csv()
reverse.main()
...
supplement.insert(0, 'data_cutoff', '2026-09-18')
...
payload = {
    ...
    'market_date': '2026-09-18', 'market_price': r.MARKET.price_hkd,
    'shares_m':r.SHARES_M,'net_cash':r.NET_CASH_USDM,'cb_principal':r.SEPT_CB_PRINCIPAL_USDM,
}
(ROOT / 'tmp/refresh_0918_workbook.json').write_text(json.dumps(payload), encoding='utf-8')
