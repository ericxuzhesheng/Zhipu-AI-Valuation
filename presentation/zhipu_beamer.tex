  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item 到 2026-08-28，股价从 IPO 价 HK\$116.20 升至 HK\$1,090。
      \end{itemize}
    \end{column}
    \begin{column}{0.48\textwidth}
      \textbf{为什么传统分析会卡住}
      \begin{itemize}
        \item 盈利为负，市盈率没有解释力。
        \item 账面权益为负，市净率也不合适。
        \item 收入仍小，但市场已经给出约 US\$62.3bn 的 equity value。
      \end{itemize}
    \end{column}
  \end{columns}
...
\begin{frame}{核心结论不是“泡沫”二字能概括}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.32\textwidth}
      \textbf{预期结论}
      \begin{itemize}
        \item 市价隐含约 312x FY2026E revenue。
...
\begin{frame}{股价走势把估值问题推到极端}
  ...
  \vspace{0.2em}
  \footnotesize
  智谱股价较 IPO 约 +838\%，而 MiniMax 上市后明显回落。橙色线是能力事件，绿色线是 IPO、指数纳入、Stock Connect、HKEX Tech 100 等资金流或生态事件。
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        ...
        \item 即使把这些变量调得更乐观，也难以靠普通 DCF 接近 HK\$1,090。
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        ...
        \item 每个格子显示要支撑 US\$62.3bn equity value 所需的 2035 年收入。
        \item 在 WACC 13.5\%、终值利润率 40\% 下，隐含 2035 年收入约 US\$71bn。
      \end{itemize}
...
\begin{frame}{可比估值先把收入口径分开}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      \textbf{交叉验证}
      \begin{itemize}
        \item OpenAI、Anthropic 与 Mistral 的私有交易区间为 20.5--39.0x，对应每股约 HK\$72--136。
        \item MiniMax 的可审计倍数为 81.4x LTM，对应约 HK\$285。
        \item 两种检查都明显低于 HK\$1,090，估值缺口没有消失。
      \end{itemize}
      \begin{block}{估值含义}
        市场价格仍要依靠很大的右尾结果才能成立。
      \end{block}
...
\begin{frame}{三层可比回答三个不同的问题}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \textbf{读图顺序}
      \begin{itemize}
        \item 先看基础模型同行。智谱为 311.5x，MiniMax 为 81.4x；私有交易中位数约 34.1x。
        \item 港股邻近组帮助判断本地 AI 资产的估值位置，但收入结构不同。
        \item 商业化参照组显示成熟 AI 软件的收入倍数，不参与核心中位数。
      \end{itemize}
      \smallnote{图中每家公司都保留估值日、收入周期和来源，倍数由数据表计算。}
    \end{column}
  \end{columns}
\end{frame}
