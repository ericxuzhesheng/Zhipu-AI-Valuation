...
HK\$240--457, but the closest listed peer implies HK\$1{,}109. The reverse DCF requires about US\$92bn of 2035
...
market capitalisation of US\$71.3bn. Against the paper's US\$700m FY2026E revenue estimate, that is an
equity-value-to-revenue multiple of 101.9$\times$
...
\label{sec:rel}
On FY2026E revenue Zhipu trades near \textbf{101.9$\times$ equity value / revenue}. We use three groups because
their businesses and revenue bases differ. MiniMax, the closest listed lab, had a 31~August~2026 equity value of
US\$15.63bn and US\$165.2m of LTM revenue, or \textbf{94.6$\times$}. Private transactions imply
...
the only transaction range carried into the football field. It gives HK\$240--457 per Zhipu share; MiniMax's
listed multiple gives HK\$1{,}109.
...
from frontier-model labs, so both groups are diagnostic context only. The updated evidence is more nuanced
than a simple ``every comparable says overvalued'' claim: Zhipu is only 1.08 times MiniMax, but 2.99 times the
private-reference median. The listed peer therefore corroborates much of the market price, while the
transactions and DCF still demand caution (Figure~\ref{fig:comps}).
...
and MiniMax's 94.6$\times$ LTM multiple gives about HK\$1{,}109. That listed-peer anchor is only 7.2\% below
Zhipu's HK\$1{,}195 close, so the evidence no longer supports dismissing the quote as peerless. It supports a
narrower conclusion: public markets value both listed foundation-model labs far above private transactions and
cash-flow scenarios, making the price unusually dependent on a durable right-tail option.
...
\citep{ernie50,ernie51,qwen35,qwen36,qwen38,tencenthy,meituanlongcat,geminiio2026,geminiflash,metamuse,musespark11,gpt55,gpt56,grokchatbot,grokofficial}.
Weekend announcements use the next trading day. MSFT and TSLA are listed-equity proxies for OpenAI and xAI;
their returns are therefore descriptive, not issuer-level causal estimates. Same-week Qwen3.8 variants are
catalogued but not double-counted after the 3~August flagship launch. Qwen3.8-Flash (26~August), Gemini
Transcribe/Omni 1.1 (26--27~August), Tencent Hy4 (28~August), GLM-5.3-Flash, the H1 results, and MSCI
rebalance lack a complete post-event window at the cutoff and remain outside the table.}
...
a HK\$1{,}195 pre-results close. MiniMax's public multiple nevertheless implies HK\$1{,}109: the premium is not
unique to Zhipu, but attaches to listed foundation-model scarcity. The reverse DCF states the wager---about
