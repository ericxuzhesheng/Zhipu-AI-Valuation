...
\institute{公司金融课程论文}
\date{13 September 2026 · 行情截至 9 月 11 日}
  ...
  \begin{columns}[T,onlytextwidth]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item 截至 2026-09-11 收市，股价从 IPO 价 HK\$116.20 升至 HK\$793。
...
\begin{frame}{核心结论不是“泡沫”二字能概括}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.32\textwidth}
      \textbf{预期结论}
      \begin{itemize}
        \item 市价对应约 226.0x LTM revenue。
        \item 基准反向 DCF 要求 2035 年收入 US\$60.5bn。
        \item 2026--2035 年收入 CAGR 约 64.1\%。
      \end{itemize}
    \end{column}
    \begin{column}{0.32\textwidth}
      \textbf{机制结论}
...
\begin{frame}{股价走势把估值问题推到极端}
  ...
  \vspace{0.2em}
  \footnotesize
  截至 9 月 11 日，智谱较 IPO 上涨约 582\%，MiniMax 上涨约 64\%。8 月 31 日智谱收涨 9.6\%；半年报在收市后发布，不能解释这段涨幅，MSCI 月末再平衡则可能贡献了部分收盘资金流。
\end{frame}
...
\begin{frame}{DCF 的结果只解释了市价的一小部分}
  ...
  \begin{table}
    ...
    \begin{tabular}{lrrrr}
      ...
      \midrule
      概率加权 & -- & -- & 5.3 & 88.0 \\
      市场价格（9/11） & -- & -- & 47.3 & 793.0 \\
      \bottomrule
    \end{tabular}
  \end{table}
  \textbf{讲解重点：} 概率加权 DCF 约为市价的 11.1\%。这不是在宣告“正确价格”，而是在量化市场价格中有多少依赖尚未兑现的右尾结果。
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        ...
        \item 典型单变量区间约 HK\$43--104，仍难以靠常规调参接近 HK\$793。
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        ...
        \item 每个格子显示要支撑 US\$47.3bn equity value 所需的 2035 年收入。
        \item 基准组合（13.5\% WACC、40\% 终值利润率）要求 2035 年收入 US\$60.5bn。
        \item 这相当于 2027 年增长 129.7\%，2026--2035 年 CAGR 64.1\%。
      \end{itemize}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \begin{itemize}
        ...
        \item 智谱 LTM 收入 US\$209.5m，对应 226.0x；MiniMax 为 73.2x。
        \item MiniMax 倍数应用于智谱 LTM 收入，对应每股约 HK\$257。
      \end{itemize}
      \begin{block}{估值含义}
...
\begin{frame}{三层可比回答三个不同的问题}
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.38\textwidth}
      ...
      \textbf{读图顺序}
      \begin{itemize}
        \item 上市基础模型同行采用同一 LTM 规则：智谱 226.0x，MiniMax 73.2x。
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.32\textwidth}
      ...
      \begin{itemize}
        ...
        \item MiniMax 的 73.2x 倍数显示板块有共同溢价，但远低于智谱的 226.0x。
...
\begin{frame}{9 月 11 日更新口径}
  \begin{itemize}
    \item 11 条行情序列均更新至 9 月 11 日收盘；智谱 HK\$793，MiniMax HK\$270。
    \item 原五事件样本与 bootstrap 保留 8 月 31 日口径。新增窗口单独观察。
    \item GLM-5.3-Flash：两日反应 +9.1\%，完整后续漂移 -25.9\%。
    \item MiniMax 半年报：两日反应 -5.7\%，完整后续漂移 -23.2\%。
    \item 智谱半年报以 9 月 1 日为 Day 0，漂移仅有 7/9 个交易日。
    \item 经营假设与净现金桥沿用原值；9 月 13 日融资仍有交割条件，未计为到账。
  \end{itemize}
  \smallnote{MiniMax、闻歌按原股本及汇率基础重估；其他可比与月度 beta 保留原日期。来源和完整窗口见 UPDATE\_2026-09-11.md。}
\end{frame}

\begin{frame}{9 月 13 日融资：已签约，仍待交割条件满足}
  \begin{itemize}
    \item 配售最多 2,196.5 万股，HK\$714/股；较 9 月 11 日收盘折价 9.96\%。
    \item 配售预计净款 HK\$156.64 亿；条件满足后预计 9 月 16 日交割。
    \item 可转债本金 RMB201.4 亿、美元结算；预计净款 US\$30.11 亿。
    \item 零息，2027 年 9 月到期；初始转股价 HK\$892.50，较收盘溢价 12.55\%。
    \item 全额配售与初始转股后股数约 5.140 亿，原持股比例合计摊薄约 9.40\%。
  \end{itemize}
  \begin{block}{估值处理}
    截止日不把款项当成已到账。交割后需同时处理现金、债务与股本；零息并不免除到期偿付。HK\$88 仍是原假设参考值。
  \end{block}
  \smallnote{来源：9 月 13 日港交所配售及可转债公告。两项交易各有条件、互不以对方完成为条件。}
\end{frame}
...
\begin{frame}{为什么又跌回来：解释压力，保留因果边界}
  \begin{itemize}
    \item 8 月 31 日至 9 月 11 日：智谱 -33.6\%，MiniMax -22.6\%。
    \item 两者共同回撤；智谱多跌约 11 个百分点，单凭比较不能确认原因。
    \item 智谱毛利率降至 26.4\%，经营与调整后亏损扩大；仍有 226 倍 LTM 溢价。
    \item GLM-5.3-Flash 短窗 +9.1\%，后续漂移 -25.9\%：发布上涨不保证持续。
  \end{itemize}
  \begin{block}{截至 9 月 13 日的结论}
    高预期面对利润与资金需求压力，价格容易重估。融资折价体现新增资本的成本；周末公告发生在这段跌幅之后，不能倒推为已证实的下跌原因。
  \end{block}
  \smallnote{截止日没有公告后的交易日；不使用 9 月 14 日行情。详见 UPDATE\_2026-09-13.md。}
\end{frame}

\begin{frame}{最后的结论}
  \begin{block}{一句话}
    H1 2026 显示 API 收入已形成显著商业化牵引，却尚未证明规模能转化为利润；HK\$793 的股价仍主要在定价能力迭代带来的右尾期权。
  \end{block}
  \vspace{0.4em}
  \textbf{论文的三个贡献}
  \begin{itemize}
