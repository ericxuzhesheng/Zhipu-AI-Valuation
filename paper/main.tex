...
had risen about 10-fold to a market capitalisation near US\$68bn---an equity value roughly 342 times its
...
pre-profit issuers. By 28~August~2026, its shares had compounded roughly 10-fold to a market
capitalisation near US\$68bn on FY2026 expected revenue of about US\$200m---an implied equity-value-to-revenue
multiple near 342$\times$ \citep{tushare2026,prospectus2513,prospectus100}. The broader frontier-AI financing wave has
...
then re-rated to HK\$1{,}090 by 28~August~2026 ($\approx +928\%$ versus IPO; realised annualised volatility
...
other Chinese frontier labs (StepStar, 01.AI) may similarly seek public capital. These listings reinforce Zhipu's Chapter~18C first-mover premium. \emph{For this paper's valuation, MiniMax is the only
direct listed comparable} (Section~\ref{sec:rel}).
...
\begin{table}[H]
\centering
\caption{Three-scenario DCF (USD unless noted; per-share in HKD). Market row at 2026-08-31.}
\label{tab:dcf}
\begin{tabular}{lcccc}
\toprule
...
\midrule
\textit{Market (2026-08-31)} & --- & --- & \textit{US\$68.3B} & \textit{1{,}195} \\
\bottomrule
\end{tabular}
\end{table}

The probability-weighted intrinsic value is about HK\$32 per share---roughly \textbf{3.0\% of the market
price}. No reasonable point estimate of fundamentals approaches the quote.

\subsection{Reverse DCF: what the price requires}
We invert the model to ask what justifies the US\$68bn equity value. At WACC~13.5\% and 40\% terminal
...
\caption{Reverse-DCF sensitivity: 2035 revenue (US\$bn) required to justify the US\$68bn equity value
...
\label{sec:rel}
On FY2026E revenue Zhipu trades near \textbf{342$\times$ equity value / revenue}. We use three groups because
their businesses and revenue bases differ. MiniMax, the closest listed lab, has a 28~August~2026 equity value of
US\$13.45bn and US\$165.2m of LTM revenue, or \textbf{81.4$\times$}. Private transactions imply
...
the only one carried into the football field. It gives HK\$72 to HK\$136 per Zhipu share; MiniMax's multiple
gives HK\$285.

SenseTime, Phancy, and Wenge AI form a Hong Kong adjacent-AI group; Palantir, Cloudflare, and Snowflake are
commercialisation references \citep{hkcomps2026,softwarecomps2026}. Their economics and revenue periods differ
from frontier-model labs, so both groups are diagnostic context only. Even so,
Zhipu stands at 3.8 times MiniMax and about nine times the private-reference median, consistent with the
reverse-DCF reading (Figure~\ref{fig:comps}).
...
range from 20.5$\times$ to 39.0$\times$ feeds the football field; MiniMax uses LTM revenue, Zhipu uses FY2026E, and each
other revenue period is recorded in \texttt{data/valuation\_comps.csv}.}
\label{fig:comps}
\end{figure}
...
DCF spans HK\$12--80, the 20.5--39.0$\times$ private-deal range gives HK\$72--136, and MiniMax's 81.4$\times$
LTM multiple gives about HK\$285. Every anchor remains far left of the HK\$1,090 quote; even the highest
listed-peer cross-check is about 74\% below the market. The quote therefore depends on the right-tail option
of Section~\ref{sec:val}, not a modest change to a conventional multiple.
...
\label{sec:revheat}
Table~\ref{tab:revheat} reports the 2035 revenue (US\$bn) required to justify the US\$68bn equity value
...
28~August~2026 market-capitalisation snapshots}. SEC, company investor relations, and public market data;
